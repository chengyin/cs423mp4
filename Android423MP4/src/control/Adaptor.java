...
public class Adaptor {
	...
	double j2 = stateManager.getLocalState().getJobQueueRemaining();
	
	int x = (int) ((s1*j1-s2*j2)/(s1+s2));
	
	if(Math.abs(x/j1) > 0.25)
	{
	    String message = ""+x;
	    transferManager.sendRequest(message);
	    String control_message = transferManager.receiveRequest();
	    String[] inf = control_message.split("");
	    if(x>0)//local need to do more jobs
	    {
		for(int i=0;i<inf.length;i=i+2)
		{
		    Job job = new Job();
		    job.x = inf[i];
		    job.y = inf[i+1];
